...
async def create_a_category(data: CreateCategoryRequest,db: AsyncSession = Depends(get_db), current_user=Depends(can_access_dashboard)):
...
async def update_a_category(category_id: UUID,data: UpdateCategoryRequest,db: AsyncSession = Depends(get_db),current_user=Depends(can_access_dashboard)):
